...
const ChatGPT = () => {
  ...
  useEffect(() => {
    ...
    const productsUnsubscribe = onSnapshot(
      ...
      async (snapshot) => {
        ...
        setProductsCount((prev) => ({
          ...prev,
          chatgpt: productsWithProviders.length,
        }));
      },
      (err) => {
        setError("Error al cargar productos de ChatGPT");
        console.error(err);
      }
    );
    ...
    const accountsQuery = query(
      collection(db, "chatgpt_accounts"),
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      navigate(`/search?q=${encodeURIComponent(searchQuery)}&category=ChatGPT`);
    }
  };
      ...
      <div className="flex flex-1 relative">
        ...
        >
          <section
            ...
            style={{
              backgroundImage:
                "url('https://images.unsplash.com/photo-1677442136019-2d7430f771bb?ixlib=rb-4.0.3&auto=format&fit=crop&w=1350&q=80')",
            }}
          >
            ...
            <div className="relative z-10 text-center px-4">
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-3 drop-shadow-lg">
                Cuentas Premium de ChatGPT
              </h1>
              <p className="text-base sm:text-lg md:text-xl text-gray-200 max-w-2xl mx-auto drop-shadow-md">
                Accede a las funciones avanzadas de ChatGPT Plus para conversaciones más inteligentes y productivas
              </p>
            </div>
          </section>
                          ...
                          className={`w-full py-2 px-3 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 shadow-lg transition-all duration-200
                            ${
                              Number(product.stock) > 0 || product.status === "A pedido"
                                ? "bg-gradient-to-r from-green-500 via-green-600 to-lime-500 hover:from-green-600 hover:to-lime-600 text-white"
