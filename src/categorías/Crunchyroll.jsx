...
const Crunchyroll = () => {
    ...
    return (
      <div className="flex items-center space-x-1">
        {[...Array(fullStars)].map((_, i) => (
          <FiStar key={`full-${i}`} className="text-orange-500" size={16} fill="currentColor" />
        ))}
        {hasHalfStar && (
          <FiStar
        ...
        {[...Array(emptyStars)].map((_, i) => (
          <FiStar key={`empty-${i}`} className="text-gray-400" size={16} />
        ))}
      </div>
    );
  };
  ...
  useEffect(() => {
    ...
    const productsUnsubscribe = onSnapshot(
      ...
      async (snapshot) => {
        ...
        setProductsCount((prev) => ({
          ...prev,
          crunchyroll: productsWithProviders.length,
        }));
      },
      (err) => {
        setError("Error al cargar productos de Crunchyroll");
        console.error(err);
      }
    );
    ...
    const accountsQuery = query(
      collection(db, "crunchyroll_accounts"),
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      navigate(`/search?q=${encodeURIComponent(searchQuery)}&category=Crunchyroll`);
    }
  };
  ...
  const renderProductStatus = (product) => {
    ...
    if (Number(product.stock) > 0) {
      return (
        <div className="absolute top-0 left-0 w-24 h-8 bg-orange-500 text-white flex items-center justify-center transform -rotate-45 -translate-x-6 translate-y-3 shadow-md rounded-br-lg">
          <span className="text-xs font-bold uppercase tracking-tight">
            Stock: {product.stock}
          </span>
        </div>
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-gray-900 to-black text-gray-200">
        <FiLoader className="animate-spin text-5xl text-orange-500 mb-4" />
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-gray-900 to-black p-4">
        <FiAlertCircle className="text-5xl text-orange-500 mb-4" />
        ...
        <button
          ...
          className="px-6 py-2 bg-orange-500 text-white rounded-full hover:bg-orange-600 transition-all shadow-lg"
        >
          Recargar
        </button>
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black text-gray-200 flex flex-col">
        ...
        <div className="container mx-auto px-4 py-4 flex items-center justify-between flex-wrap">
          <div className="flex items-center space-x-3">
            ...
            <Link to="/" className="flex items-center space-x-2">
              ...
              <span className="text-xl font-semibold text-orange-500 hidden sm:block">
                BlackStreaming
              </span>
            </Link>
          </div>
          ...
          <div className="flex items-center relative w-full sm:w-auto sm:max-w-xs md:max-w-md mt-3 sm:mt-0 order-3 sm:order-2 sm:mx-3">
            <input
              type="text"
              placeholder="Buscar en Crunchyroll..."
              className="w-full px-4 py-2 rounded-full bg-gray-800/50 text-gray-200 border border-gray-700 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all placeholder-gray-500"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
            <button
              onClick={handleSearch}
              className="absolute right-3 text-gray-400 hover:text-orange-500 transition-all"
            >
              <FiSearch size={20} />
            </button>
          </div>
            ...
            {user ? (
              <div className="flex items-center space-x-2 md:space-x-4">
                <span className="text-sm font-medium text-gray-300 hidden sm:flex items-center">
                  <FiUser className="mr-2 text-orange-500" /> {user.name}
                </span>
                <div className="flex items-center space-x-2">
                  ...
                  <button
                    ...
                    className="flex items-center space-x-1 px-3 py-2 rounded-full bg-orange-600/80 hover:bg-orange-700 transition-all text-sm text-white"
                  >
                    <FiLogOut size={18} />
              ...
              <div className="flex items-center space-x-2">
                <button
                  ...
                  className="px-4 py-2 text-sm text-gray-200 hover:text-orange-500 transition-all"
                >
                  Ingresar
                </button>
                <button
                  onClick={() => navigate("/register")}
                  className="px-4 py-2 rounded-full bg-orange-500 text-white hover:bg-orange-600 transition-all text-sm"
                >
                  Registrarse
                </button>
              </div>
      ...
      <div className="flex flex-1 relative">
        ...
        >
          <div className="flex flex-col h-full">
            ...
            <nav className="flex-1 overflow-y-auto p-4 space-y-2">
              ...
              <Link
                to="/crunchyroll"
                className="flex items-center justify-between px-4 py-3 rounded-xl bg-orange-600/30 text-white border border-orange-500/30"
              >
                <div className="flex items-center space-x-2">
            ...
            {user && (
              <div className="p-4 border-t border-gray-800/50">
                <div className="flex items-center space-x-3 p-3 bg-gray-800/50 rounded-xl">
                  <div className="w-12 h-12 rounded-full bg-orange-500/30 flex items-center justify-center text-white font-bold text-lg">
        ...
        >
          <section
            ...
            style={{
              backgroundImage:
                "url('https://images.unsplash.com/photo-1618336753974-8d9f3044f8b6?ixlib=rb-4.0.3&auto=format&fit=crop&w=1350&q=80')",
            }}
          >
            ...
            <div className="relative z-10 text-center px-4">
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-3 drop-shadow-lg">
                Cuentas Premium de Crunchyroll
              </h1>
              <p className="text-base sm:text-lg md:text-xl text-gray-200 max-w-2xl mx-auto drop-shadow-md">
                Disfruta de anime, manga y series asiáticas sin anuncios con nuestras cuentas premium de Crunchyroll
              </p>
            </div>
          </section>
          ...
          <section className="container mx-auto px-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 space-y-4 sm:space-y-0">
              ...
              <button
                ...
                className="text-sm sm:text-base text-orange-500 hover:text-orange-400 transition-all underline underline-offset-4"
              >
                Términos Generales
              </button>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
                {filteredProducts.map((product) => (
                  <div
                    ...
                    style={{
                      boxShadow: "0 2px 10px 0 rgba(0,0,0,0.20), 0 1.5px 8px 0 rgba(255,165,0,0.05)",
                  ...
                  >
                    ...
                    <div className="flex flex-col flex-grow p-4 pb-3">
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="uppercase font-bold tracking-wide text-xs text-orange-500 bg-orange-900/20 px-2 py-0.5 rounded-md">
                          {product.accountType}
                        </span>
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        <span className="flex items-center text-xs text-gray-300 gap-1">
                          <FiUserCheck className="text-orange-500" size={13} />
                          {product.provider}
                        </span>
                        <span className="flex items-center text-xs text-gray-400 gap-1">
                          <FiActivity className="text-orange-500" size={13} />
                          {product.status}
                        </span>
                      </div>
                      ...
                      <div className="flex-1 flex flex-col justify-end">
                        ...
                        <div className="flex gap-2 mb-2">
                          ...
                          >
                            <FiInfo className="text-orange-500" size={14} />
                            <span>Detalles</span>
                          </button>
                          <button
                            ...
                            style={{ backdropFilter: "blur(2px)" }}
                          >
                            <FiFileText className="text-orange-500" size={14} />
                            <span>Términos</span>
                          </button>
                        </div>
                        <button
                          ...
                          className={`w-full py-2 px-3 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 shadow-lg transition-all duration-200
                            ${
                              Number(product.stock) > 0 || product.status === "A pedido"
                                ? "bg-gradient-to-r from-orange-500 via-orange-600 to-yellow-500 hover:from-orange-600 hover:to-yellow-600 text-white"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="bg-gray-800/90 rounded-xl shadow-2xl p-6 sm:p-8 w-full max-w-md border border-gray-700/50">
            <h3 className="text-xl sm:text-2xl font-bold text-white mb-4 flex items-center">
              <FiShoppingCart className="mr-2 text-orange-500" /> Confirmar Compra
            </h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 rounded-md bg-gray-700/50 text-white border border-gray-600 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 transition-all"
            ...
            <div className="flex justify-end space-x-3 mt-6">
              ...
              <button
                ...
                disabled={loading}
                className={`px-4 py-2 bg-orange-500 text-white rounded-md hover:bg-orange-600 transition-all flex items-center ${
      ...
      {detailModal && !termsModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="bg-gray-800/90 rounded-xl shadow-2xl p-6 sm:p-8 w-full max-w-md border border-gray-700/50">
            <h3 className="text-xl sm:text-2xl font-bold text-white mb-4 flex items-center">
              <FiInfo className="mr-2 text-orange-500" /> Detalles del Producto
            </h3>
      ...
      {detailModal && termsModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="bg-gray-800/90 rounded-xl shadow-2xl p-6 sm:p-8 w-full max-w-md border border-gray-700/50">
            <h3 className="text-xl sm:text-2xl font-bold text-white mb-4 flex items-center">
              <FiFileText className="mr-2 text-orange-500" /> Términos y Condiciones
            </h3>
            <div className="space-y-3 text-gray-300">
              <p>
      ...
      {generalTermsModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="bg-gray-800/90 rounded-xl shadow-2xl p-6 sm:p-8 w-full max-w-md border border-gray-700/50">
            <h3 className="text-xl sm:text-2xl font-bold text-white mb-4 flex items-center">
              <FiFileText className="mr-2 text-orange-500" /> Términos Generales
            </h3>
      ...
      {notificationModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="bg-gray-800/90 rounded-xl shadow-2xl p-6 sm:p-8 w-full max-w-md border border-gray-700/50">
            <h3 className="text-xl sm:text-2xl font-bold text-white mb-4 flex items-center">
              <FiInfo className="mr-2 text-orange-500" /> Notificación
            </h3>
