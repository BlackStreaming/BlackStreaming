...
// Notification Modal (Reused from DashboardAdmin)
...
// Edit Profile Modal (Inspired by EditBalanceModal)
...
const DashboardUser = () => {
    ...
    const fetchNotifications = () => {
      const unsubscribe = onSnapshot(doc(db, "notifications", "adminMessage"), (docSnapshot) => {
        if (docSnapshot.exists()) {
          ...
          const userRole = "user"; // Assuming user role; adjust if dynamic roles are needed
  ...
  const handleRenewal = async (order) => {
    try {
      ...
      const newOrder = {
        customerId: userId,
        customerName: order.client?.customerName || userName,
        ...
        providerPhone: order.providerPhone || "51999999999",
        status: "completed",
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              ...
              <div className="bg-gray-700/50 backdrop-blur-sm rounded-2xl p-4 border border-gray-600/50 shadow-lg">
                ...
                <p className="text-3xl font-bold text-white">
                  {orders.filter((o) => new Date(o.endDate) > new Date() && o.status === "completed").length}
                </p>
                <button
