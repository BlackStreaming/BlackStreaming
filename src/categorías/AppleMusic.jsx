...
const AppleMusic = () => {
  ...
  useEffect(() => {
    ...
    const productsUnsubscribe = onSnapshot(
      ...
      async (snapshot) => {
        ...
        setProductsCount((prev) => ({
          ...prev,
          applemusic: productsWithProviders.length,
        }));
      },
      (err) => {
        setError("Error al cargar productos de Apple Music");
        console.error(err);
      }
    );
    ...
    const ratingsUnsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.docs.forEach((doc) => {
          ...
          ratingsData[productId].push(Number(rating.rating) || 0);
        });
    ...
    const accountsQuery = query(
      collection(db, "applemusic_accounts"),
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      navigate(`/search?q=${encodeURIComponent(searchQuery)}&category=Apple Music`);
    }
  };
      ...
      <header className="sticky top-0 z-50 bg-gray-900/80 backdrop-blur-md border-b border-gray-800/50">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between flex-wrap">
          ...
          <div className="flex items-center relative w-full sm:w-auto sm:max-w-xs md:max-w-md mt-3 sm:mt-0 order-3 sm:order-2 sm:mx-3">
            <input
              type="text"
              placeholder="Buscar en Apple Music..."
      ...
      <div className="flex flex-1 relative">
        ...
        >
          <section
            ...
            style={{
              backgroundImage:
                "url('https://images.unsplash.com/photo-1514327605112-b887c0e61c0a?ixlib=rb-4.0.3&auto=format&fit=crop&w=1350&q=80')",
            }}
          >
            ...
            <div className="relative z-10 text-center px-4">
              ...
              <p className="text-base sm:text-lg md:text-xl text-gray-200 max-w-2xl mx-auto drop-shadow-md">
                Disfruta de música sin límites con nuestras suscripciones a Apple Music
              </p>
            </div>
          </section>
