...
import { AuthContext } from "../AuthContext"; // Asegúrate de que este archivo esté en la ruta correcta
...
const Home = () => {
  ...
  const { user, balance, role, loading } = useContext(AuthContext); // Usar AuthContext
  ...
  // Fetch admin notification only if user is authenticated
  useEffect(() => {
    if (!loading && user) {
