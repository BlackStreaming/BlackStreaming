...
const Tidal = () => {
    ...
    const productsUnsubscribe = onSnapshot(
      ...
      async (snapshot) => {
        ...
        setProductsCount((prev) => ({
          ...prev,
          tidal: productsWithProviders.length,
        }));
      },
      (err) => {
        setError("Error al cargar productos de Tidal");
        console.error(err);
      }
    );
      ...
      if (selectedProduct.status === "En stock") {
        accountData = await runTransaction(db, async (transaction) => {
          ...
          const orderData = {
            ...
            renewalPrice: selected.NodesRenewalPrice,
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      navigate(`/search?q=${encodeURIComponent(searchQuery)}&category=Tidal`);
    }
  };
  ...
  const renderProductStatus = (product) => {
    ...
    return (
      <div className="absolute top-0 left-0 w-24 h-8 bg-red-500 text-white flex items-center justify-center transform -rotate-45 -translate-x-6 translate-y-3 shadow-md attorno-br-lg">
        <span className="text-xs font-bold uppercase tracking-tight">
          Agotado
        </span>
      </div>
      ...
      <div className="flex flex-1 relative">
        ...
        >
          <section
            ...
            style={{
              backgroundImage:
                "url('https://images.unsplash.com/photo-1514320291840-2e0a9bf2a9ae?ixlib=rb-4.0.3&auto=format&fit=crop&w=1350&q=80')",
            }}
          >
            ...
            <div className="relative z-10 text-center px-4">
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-3 drop-shadow-lg">
                Cuentas Premium de Tidal
              </h1>
              <p className="text-base sm:text-lg md:text-xl text-gray-200 max-w-2xl mx-auto drop-shadow-md">
                Disfruta de música en alta fidelidad y contenido exclusivo con nuestras cuentas premium de Tidal
              </p>
            </div>
          </section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
                {filteredProducts.map((product) => (
                  <div
                    ...
                    style={{
                      boxShadow: "0 2px 10px 0 rgba(0,0,0,0.20), 0 1.5px 8px 0 rgba(0,255,0,0.05)",
                          ...
                          className={`w-full py-2 px-3 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 shadow-lg transition-all duration-200
                            ${
                              Number(product.stock) > 0 || product.status === "A pedido"
                                ? "bg-gradient-to-r from-green-500 via-green-600 to-teal-500 hover:from-green-600 hover:to-teal-600 text-white"
              ...
              <div className="flex items-center">
                ...
                <label htmlFor="termsCheck" className="ml-2 text-sm text-gray-300">
                  ...
                  <button
                    ...
                    className="text-green-400 hover:text-blue-300 underline underline-offset-2"
                  >
