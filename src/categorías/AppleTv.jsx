...
const AppleTV = () => {
  ...
  // Render star rating for provider
    ...
    return (
      <div className="flex items-center space-x-1">
        {[...Array(fullStars)].map((_, i) => (
          <FiStar key={`full-${i}`} className="text-gray-200" size={16} fill="currentColor" />
        ))}
        {hasHalfStar && (
          <FiStar
        ...
        {[...Array(emptyStars)].map((_, i) => (
          <FiStar key={`empty-${i}`} className="text-gray-600" size={16} />
        ))}
      </div>
    );
  };
      ...
      async (snapshot) => {
        const productsData = snapshot.docs.map((doc) => {
          ...
          return {
            ...
            stock: Number(data.stock || data.available || 0),
            accountDetails: data.email || data.details || "Detalles no disponibles",
        ...
        setProductsCount((prev) => ({
          ...prev,
          appletv: productsWithProviders.length,
        }));
      },
      (err) => {
        setError("Error al cargar productos de AppleTV");
        console.error(err);
      }
    );
    ...
    const ratingsUnsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.docs.forEach((doc) => {
          ...
          if (!ratingsData[providerId]) {
            ratingsData[providerId] = [];
          }
          ratingsData[providerId].push(Number(rating.rating) || 0);
        });
        const averageRatings = {};
        Object.keys(ratingsData).forEach((providerId) => {
          const ratings = ratingsData[providerId];
          const average = ratings.length
            ? ratings.reduce((sum, r) => sum + r, 0) / ratings.length
            : 1;
          averageRatings[providerId] = Math.min(Math.max(average, 1), 5);
        });
        setProviderRatings(averageRatings);
      },
      (err) => {
        console.error("Error al cargar calificaciones de proveedores:", err);
      }
    );

    const accountsQuery = query(
      collection(db, "appletv_accounts"),
    ...
    try {
      ...
      if (selectedProduct.status === "En stock") {
        accountData = await runTransaction(db, async (transaction) => {
          ...
          if (productData.availableAccounts <= 0 && productData.stock <= 0) {
  ...
  const handleSearch = (e) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      navigate(`/search?q=${encodeURIComponent(searchQuery)}&category=AppleTV`);
    }
  };
  ...
  const renderProductStatus = (product) => {
    ...
    if (Number(product.stock) > 0) {
      return (
        <div className="absolute top-0 left-0 w-24 h-8 bg-gray-200 text-gray-900 flex items-center justify-center transform -rotate-45 -translate-x-6 translate-y-3 shadow-md rounded-br-lg">
          <span className="text-xs font-bold uppercase tracking-tight">
            Stock: {product.stock}
          </span>
        </div>
    ...
    return (
      <div className="absolute top-0 left-0 w-24 h-8 bg-gray-600 text-white flex items-center justify-center transform -rotate-45 -translate-x-6 translate-y-3 shadow-md rounded-br-lg">
        <span className="text-xs font-bold uppercase tracking-tight">
          Agotado
        </span>
      </div>
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-gray-900 to-black text-gray-200">
        <FiLoader className="animate-spin text-5xl text-gray-200 mb-4" />
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-gray-900 to-black p-4">
        <FiAlertCircle className="text-5xl text-gray-200 mb-4" />
        ...
        <button
          ...
          className="px-6 py-2 bg-gray-200 text-gray-900 rounded-full hover:bg-gray-300 transition-all shadow-lg"
        >
          Recargar
        </button>
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black text-gray-200 flex flex-col">
        ...
        <div className="container mx-auto px-4 py-4 flex items-center justify-between flex-wrap">
          <div className="flex items-center space-x-3">
            ...
            <Link to="/" className="flex items-center space-x-2">
              ...
              <span className="text-xl font-semibold text-gray-200 hidden sm:block">
                BlackStreaming
              </span>
            </Link>
          </div>
          ...
          <div className="flex items-center relative w-full sm:w-auto sm:max-w-xs md:max-w-md mt-3 sm:mt-0 order-3 sm:order-2 sm:mx-3">
            <input
              type="text"
              placeholder="Buscar en Apple TV..."
              className="w-full px-4 py-2 rounded-full bg-gray-800/50 text-gray-200 border border-gray-700 focus:ring-2 focus:ring-gray-200 focus:border-gray-200 transition-all placeholder-gray-500"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
            <button
              onClick={handleSearch}
              className="absolute right-3 text-gray-400 hover:text-gray-200 transition-all"
            >
              <FiSearch size={20} />
            </button>
          </div>
            ...
            {user ? (
              <div className="flex items-center space-x-2 md:space-x-4">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button
                    ...
                    title="Dashboard"
                  >
                    <FiUser className="text-gray-200" size={20} />
                  </button>
                  <button
                    onClick={() => {
                      auth.signOut();
                    ...
                    className="flex items-center space-x-1 px-3 py-2 rounded-full bg-gray-200/80 text-gray-900 hover:bg-gray-300 transition-all text-sm"
                  >
                    <FiLogOut size={18} />
              ...
              <div className="flex items-center space-x-2">
                <button
                  ...
                  className="px-4 py-2 text-sm text-gray-200 hover:text-gray-300 transition-all"
                >
                  Ingresar
                </button>
                <button
                  onClick={() => navigate("/register")}
                  className="px-4 py-2 rounded-full bg-gray-200 text-gray-900 hover:bg-gray-300 transition-all text-sm"
                >
                  Registrarse
                </button>
              </div>
      ...
      <div className="flex flex-1 relative">
        ...
        >
          <div className="flex flex-col h-full">
            ...
            <nav className="flex-1 overflow-y-auto p-4 space-y-2">
              ...
              >
                <div className="flex items-center space-x-2">
                  <FiPenTool className="text-teal-500" />
                  <span>Canva</span>
                </div>
                <span className="bg-gray-800/50 text-white text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center">
                  {productsCount.canva}
              ...
              <Link
                to="/appletv"
                className="flex items-center justify-between px-4 py-3 rounded-xl bg-gray-200/30 text-white border border-gray-200/30"
              >
                <div className="flex items-center space-x-2">
                  <FiTv className="text-gray-200" />
                  <span>Apple Tv</span>
                </div>
                <span className="bg-gray-800/50 text-white text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center">
            ...
            {user && (
              <div className="p-4 border-t border-gray-800/50">
                <div className="flex items-center space-x-3 p-3 bg-gray-800/50 rounded-xl">
                  <div className="w-12 h-12 rounded-full bg-gray-200/30 flex items-center justify-center text-white font-bold text-lg">
        ...
        >
          <section
            ...
            style={{
              backgroundImage:
                "url('https://images.unsplash.com/photo-1536440136628-0f1a91f70d79?w=1350&q=80')",
            }}
          >
            ...
            <div className="relative z-10 text-center px-4">
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-3 drop-shadow-lg">
                Apple TV+: Streaming Exclusivo
              </h1>
              <p className="text-base sm:text-lg md:text-xl text-gray-200 max-w-2xl mx-auto drop-shadow-md">
                Disfruta de series y películas originales de Apple TV+ en cualquier dispositivo
              </p>
            </div>
          </section>
          ...
          <section className="container mx-auto px-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 space-y-4 sm:space-y-0">
              ...
              <button
                ...
                className="text-sm sm:text-base text-gray-200 hover:text-gray-300 transition-all underline underline-offset-4"
              >
                Términos Generales
              </button>
            </div>
            ...
            {filteredProducts.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
                {filteredProducts.map((product) => (
                  <div
                    key={product.id}
                    className="relative group bg-gradient-to-br from-zinc-900/70 to-gray-950/80 rounded-2xl overflow-hidden border border-gray-700/50 shadow-xl hover:shadow-2xl transition-shadow duration-300"
                    style={{
                      boxShadow: "0 2px 10px 0 rgba(0,0,0,0.20), 0 1.5px 8px 0 rgba(255,255,255,0.05)",
                  ...
                  >
                    ...
                    <div className="flex flex-col flex-grow p-4 pb-3">
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="uppercase font-bold tracking-wide text-xs text-gray-200 bg-gray-900/20 px-2 py-0.5 rounded-md">
                          {product.accountType}
                        </span>
                        <span className="text-xs text-gray-400 flex items-center gap-1">
                          <FiClock className="text-gray-200" size={13} />
                          {parseDurationToDays(product.duration)}
                        </span>
                      </div>
                      <h3 className="font-extrabold text-lg text-white mb-2 truncate leading-tight drop-shadow">
                        {product.name}
                      </h3>
                      <div className="flex items-center gap-1 mb-1.5">
                        {renderStars(providerRatings[product.providerId] || 1)}
                        <span className="ml-1 text-gray-400 text-xs">
                          {(providerRatings[product.providerId] || 1).toFixed(1)}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 mb-2">
                        <span className="flex items-center text-xs text-gray-300 gap-1">
                          <FiUserCheck className="text-gray-200" size={13} />
                          {product.provider}
                        </span>
                        <span className="flex items-center text-xs text-gray-400 gap-1">
                          <FiActivity className="text-gray-200" size={13} />
                          {product.status}
                        </span>
                      </div>
                      <div className="flex items-center mb-2">
                        {product.renewal ? (
                          ...
                          <span className="text-yellow-400 text-xs font-medium">Sin renovación</span>
                        )}
                      </div>
                      <div className="flex-1 flex flex-col justify-end">
                        ...
                        <div className="flex gap-2 mb-2">
                          <button
                            ...
                            className="flex-1 flex items-center justify-center gap-1 py-1.5 px-3 rounded-lg bg-gray-800/60 hover:bg-gray-700/70 text-gray-100 transition-all border border-gray-700/40 text-xs font-medium shadow-inner focus:outline-none"
                            style={{ backdropFilter: "blur(2px)" }}
                          >
                            <FiInfo className="text-gray-200" size={14} />
                            <span>Detalles</span>
                          </button>
                          <button
                            ...
                            style={{ backdropFilter: "blur(2px)" }}
                          >
                            <FiFileText className="text-gray-200" size={14} />
                            <span>Términos</span>
                          </button>
                        </div>
                        <button
                          onClick={() => handlePurchase(product)}
                          className={`w-full py-2 px-3 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 shadow-lg transition-all duration-200
                            ${
                              Number(product.stock) > 0 || product.status === "A pedido"
                                ? "bg-gradient-to-r from-gray-200 via-gray-300 to-white/80 text-gray-900 hover:from-gray-300 hover:to-white"
                                : "bg-gray-700 text-gray-400 cursor-not-allowed"
                            }
                          `}
                          disabled={Number(product.stock) <= 0 && product.status !== "A pedido"}
                        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="bg-gray-800/90 rounded-xl shadow-2xl p-6 sm:p-8 w-full max-w-md border border-gray-700/50">
            <h3 className="text-xl sm:text-2xl font-bold text-white mb-4 flex items-center">
              <FiShoppingCart className="mr-2 text-gray-200" /> Confirmar Compra
            </h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2 rounded-md bg-gray-700/50 text-white border border-gray-600 focus:ring-2 focus:ring-gray-200 focus:border-gray-200 transition-all"
              ...
              <div className="flex items-center">
                <input
                  type="checkbox"
                  id="termsCheck"
                  className="h-4 w-4 text-gray-200 bg-gray-700 border-gray-600 rounded focus:ring-gray-200"
                />
                <label htmlFor="termsCheck" className="ml-2 text-sm text-gray-300">
                  Acepto los{" "}
                  <button
                    onClick={() => showTerms(purchaseModal.product)}
                    className="text-gray-200 hover:text-gray-300 underline underline-offset-2"
                  >
            ...
            <div className="flex justify-end space-x-3 mt-6">
              ...
              <button
                ...
                disabled={loading}
                className={`px-4 py-2 bg-gray-200 text-gray-900 rounded-md hover:bg-gray-300 transition-all flex items-center ${
      ...
      {detailModal && !termsModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="bg-gray-800/90 rounded-xl shadow-2xl p-6 sm:p-8 w-full max-w-md border border-gray-700/50">
            <h3 className="text-xl sm:text-2xl font-bold text-white mb-4 flex items-center">
              <FiInfo className="mr-2 text-gray-200" /> Detalles del Producto
            </h3>
      ...
      {detailModal && termsModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="bg-gray-800/90 rounded-xl shadow-2xl p-6 sm:p-8 w-full max-w-md border border-gray-700/50">
            <h3 className="text-xl sm:text-2xl font-bold text-white mb-4 flex items-center">
              <FiFileText className="mr-2 text-gray-200" /> Términos y Condiciones
            </h3>
            <div className="space-y-3 text-gray-300">
              <p>
            ...
            <div className="flex justify-end mt-6 space-x-3">
              ...
              {purchaseModal && (
                <button
                  ...
                  className="px-4 py-2 bg-gray-200 text-gray-900 rounded-md hover:bg-gray-300 transition-all"
                >
                  Volver a Compra
                </button>
              )}
      ...
      {generalTermsModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="bg-gray-800/90 rounded-xl shadow-2xl p-6 sm:p-8 w-full max-w-md border border-gray-700/50">
            <h3 className="text-xl sm:text-2xl font-bold text-white mb-4 flex items-center">
              <FiFileText className="mr-2 text-gray-200" /> Términos Generales
            </h3>
      ...
      {notificationModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="bg-gray-800/90 rounded-xl shadow-2xl p-6 sm:p-8 w-full max-w-md border border-gray-700/50">
            <h3 className="text-xl sm:text-2xl font-bold text-white mb-4 flex items-center">
              <FiInfo className="mr-2 text-gray-200" /> Notificación
            </h3>
