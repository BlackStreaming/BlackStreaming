...
const Spotify = () => {
  ...
  const [purchaseModal, setPurchaseModal] = useState(null); // Nuevo estado para el modal de compra
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const spotifyProducts = snapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));
      setProducts(spotifyProducts);
    });
  ...
  const handlePurchase = async (product) => {
    ...
    // Abrir modal para llenar datos del cliente
        setPurchaseModal({
          product: product,
          customerName: "",
          phoneNumber: "",
          duration: product.duration || "Ilimitado", // Si no tiene duración, poner "Ilimitado"
        });
      };
    
      const finalizePurchase = async () => {
        const { product, customerName, phoneNumber, duration } = purchaseModal;
        ...
        <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-70 flex justify-center items-center z-50">
          <div className="bg-white p-8 rounded-lg shadow-xl max-w-lg w-full">
            <h2 className="text-2xl font-bold mb-4 text-gray-900">
              Completar Compra - {purchaseModal.product.name}
            </h2>
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full mt-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-400 transition"
              ...
              <div>
                ...
                <input
                  type="text"
                  value={purchaseModal.duration}
                  disabled
                  className="w-full mt-1 px-4 py-2 border border-gray-300 rounded-lg bg-gray-100 text-gray-500"
            ...
            <div className="flex justify-end mt-6">
              ...
              <button
                className="ml-4 px-6 py-2 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 transition"
                onClick={closeModal}
              >
                Cancelar
              </button>
