    ...
    const Netflix = () => {
      ...
      const calculateRating = (purchaseCount) => {
        if (purchaseCount <= 0) return 1;
        if (purchaseCount <= 5) return 1 + purchaseCount * 0.5;
        if (purchaseCount <= 10) return 3 + (purchaseCount - 5) * 0.2;
        if (purchaseCount <= 20) return 4 + (purchaseCount - 10) * 0.1;
        return 5;
      };
        ...
        return (
          <div className="flex items-center space-x-1">
            {[...Array(fullStars)].map((_, i) => (
              <FiStar key={`full-${i}`} className="text-yellow-400" size={14} fill="currentColor" />
            ))}
            {hasHalfStar && (
              <FiStar
            ...
            {[...Array(emptyStars)].map((_, i) => (
              <FiStar key={`empty-${i}`} className="text-gray-400" size={14} />
            ))}
          </div>
        );
      };
      ...
      const renderProductStatus = (product) => {
        if (product.status === "A pedido") {
          return (
            <div className="absolute top-1 right-1 px-2 py-1 bg-yellow-500/80 text-white text-xs font-semibold rounded-full shadow-md backdrop-blur-sm">
              A pedido
            </div>
          );
        }
        if (product.stock > 0) {
          return (
            <div className="absolute top-1 right-1 px-2 py-1 bg-green-500/80 text-white text-xs font-semibold rounded-full shadow-md backdrop-blur-sm">
              Stock: {product.stock}
            </div>
          );
        }
        return (
          <div className="absolute top-1 right-1 px-2 py-1 bg-red-500/80 text-white text-xs font-semibold rounded-full shadow-md backdrop-blur-sm">
            Agotado
          </div>
        );
      };
                ...
                {filteredProducts.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 w-full">
                    {filteredProducts.map((product) => (
                      <div
                        key={product.id}
                        className="relative bg-gradient-to-b from-gray-800/80 to-gray-900/90 rounded-xl shadow-lg overflow-hidden transform hover:-translate-y-1 hover:shadow-xl transition-all duration-300 border border-gray-700/30 backdrop-blur-sm flex flex-col"
                      >
                        ...
                        <div className="p-3 flex flex-col flex-grow">
                          ...
                          <div className="flex items-center mb-2">
                            {renderStars(productRatings[product.id] || 1)}
                            <span className="ml-1 text-gray-400 text-xs">
                              ({(productRatings[product.id] || 1).toFixed(1)})
                            </span>
                          </div>
                          <div className="flex items-center mb-2">
                            {product.renewal ? (
                              ...
                              <p className="text-yellow-400 text-xs font-medium">Sin renovación</p>
                            )}
                          </div>
                          <div className="grid grid-cols-2 gap-1 text-xs text-gray-300 mb-3">
                            <div className="flex items-center space-x-1 bg-gray-700/30 p-1 rounded-md">
                              <FiUserCheck className="text-red-400 flex-shrink-0" size={12} />
                              <span className="truncate">{product.provider}</span>
                            </div>
                            <div className="flex items-center space-x-1 bg-gray-700/30 p-1 rounded-md">
                              <FiClock className="text-red-400 flex-shrink-0" size={12} />
                              <span>{parseDurationToDays(product.duration)}</span>
                            </div>
                            <div className="flex items-center space-x-1 bg-gray-700/30 p-1 rounded-md">
                              <FiTag className="text-red-400 flex-shrink-0" size={12} />
                              <span className="truncate">{product.accountType}</span>
                            </div>
                            <div className="flex items-center space-x-1 bg-gray-700/30 p-1 rounded-md">
                              <FiActivity className="text-red-400 flex-shrink-0" size={12} />
                          ...
                          <div className="flex justify-center items-center gap-2 mb-3">
                            <button
                              ...
                              className="relative flex-1 py-1.5 px-3 bg-gray-700/50 hover:bg-gray-600/50 text-white rounded-md text-xs flex items-center justify-center transition-all duration-200 border border-gray-600/30 group"
                            >
                              <FiInfo className="text-red-400" size={14} />
                              <span className="absolute left-1/2 transform -translate-x-1/2 bg-gray-800 text-white text-xs rounded-md py-1 px-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 md:block hidden">
                                Detalles
                              </span>
                          ...
                          <button
                            ...
                            className={`w-full py-2 px-3 rounded-md text-white font-semibold text-sm transition-all duration-200 flex items-center justify-center gap-2 ${
