...
const DashboardAdmin = () => {
  ...
  const [notificationForm, setNotificationForm] = useState({
    message: "",
    roles: { user: false, affiliate: false, provider: false, admin: false },
  });
  ...
  const handleNotificationSubmit = async (e) => {
    ...
    try {
      ...
      const { message, roles } = notificationForm;
      if (!message.trim()) {
      ...
      const notificationRef = doc(db, "notifications", "adminMessage");
      ...
      showModal("Éxito", "Notificación enviada exitosamente");
    } catch (error) {
      console.error("Error al enviar notificación:", error);
      showModal("Error", error.message || "Error al enviar la notificación");
    } finally {
...
const handleAccept = async (registrationId) => {
  try {
    ...
    await setDoc(doc(db, "users", newUser.uid), {
      ...
      phone: registration.phone || null, // Include phone number from registration
      ...
      case "notificaciones":
        return (
          <div className="bg-gray-800/50 backdrop-blur-sm p-4 sm:p-6 rounded-2xl shadow-xl max-w-2xl mx-auto border border-gray-700/50">
            <h3 className="text-xl sm:text-2xl font-bold text-white mb-6">Enviar Notificación</h3>
            <form onSubmit={handleNotificationSubmit} className="space-y-6">
              <div>
                <label className="block text-gray-300 mb-2">Mensaje de la Notificación</label>
                <textarea
                  value={notificationForm.message}
                  onChange={(e) => setNotificationForm({ ...notificationForm, message: e.target.value })}
                  className="w-full px-4 py-2 rounded-xl bg-gray-700/50 text-white border border-gray-600/50 focus:ring-2 focus:ring-cyan-400 focus:border-cyan-400 transition-all resize-y"
                  rows="5"
              ...
              <div>
                <label className="block text-gray-300 mb-2">Enviar a los siguientes roles</label>
                <div className="grid grid-cols-2 gap-4">
                  {["user", "affiliate", "provider", "admin"].map((role) => (
                    <label key={role} className="flex items-center space-x-2">
                      <input
                        type="checkbox"
                        checked={notificationForm.roles[role]}
                        onChange={(e) =>
                          setNotificationForm({
                            ...notificationForm,
                            roles: { ...notificationForm.roles, [role]: e.target.checked },
                          })
                        }
              ...
              <button
                ...
                className="w-full py-3 px-4 bg-cyan-500 hover:bg-cyan-600 text-white rounded-xl font-medium transition-all duration-300 disabled:bg-cyan-800"
              >
                  ...
                  <tbody className="bg-gray-800/50 divide-y divide-gray-600/50">
                    {sales.map((sale) => (
                      <tr key={sale.id} className="hover:bg-gray-700/50 transition-all duration-300">
                        ...
                        <td className="px-4 py-4 whitespace-nowrap text-gray-300">{sale.customerName || "Sin usuario"}</td>
        ...
        <div className="p-4 flex flex-col h-full">
          ...
          <nav className="flex-1 space-y-2">
            {[
              ...
              { id: "retiros", label: "Retiros", icon: FiDollarSign },
